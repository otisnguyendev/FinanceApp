...
from database.transactions import add_transaction, get_transactions, update_transaction, delete_transaction
...
class TransactionForm(tk.Frame):
    ...
    def create_widgets(self):
        ...
        for i, (text, bg, command) in enumerate(buttons):
            btn = tk.Button(button_frame, text=text, bg=bg, fg="white", font=("Poppins", 12, "bold"), padx=10, pady=5, command=command, width=15)
            button_frame.grid_columnconfigure(i, weight=1)
            btn.grid(row=0, column=i, padx=10)

        self.tree = ttk.Treeview(self, columns=("Loại", "Danh mục", "Số tiền", "Ngày", "Ghi chú", "Ví"), show="headings", style="Treeview")
    ...
    def load_transactions(self):
        ...
        for trans in transactions:
            ...
            self.tree.insert("", "end", iid=trans[0], values=(display_type, trans[2], trans[3], trans[4], trans[5], wallet_name))
    ...
    def edit_transaction(self):
        ...
        trans_id = selected[0]  # Lấy trans_id từ iid
        values = self.tree.item(trans_id, 'values')
        type_value = "income" if values[0] == "Thu nhập" else "expense"
        self.open_transaction_form(type_value, "Sửa giao dịch", lambda f, t, c, a, d, n, w: self.update_transaction(f, trans_id, t, c, a, d, n, w), values)
    ...
    def open_transaction_form(self, type, title, save_command, values=None):
        ...
        tk.Label(form, text=title, bg="#F5F5F5", font=("Poppins", 16, "bold"), fg="#333").grid(row=0, column=0, columnspan=2, pady=(0, 20))
        ...
        tk.Label(content_frame, text="Loại giao dịch:", bg="#F5F5F5", font=("Poppins", 12)).grid(row=0, column=0, pady=5, sticky="e")
        type_var = tk.StringVar(value=type)
        tk.Radiobutton(content_frame, text="Thu nhập", variable=type_var, value="income", bg="#F5F5F5", font=("Poppins", 12)).grid(row=0, column=1, sticky="w")
        tk.Radiobutton(content_frame, text="Chi tiêu", variable=type_var, value="expense", bg="#F5F5F5", font=("Poppins", 12)).grid(row=1, column=1, sticky="w")

        tk.Label(content_frame, text="Danh mục:", bg="#F5F5F5", font=("Poppins", 12)).grid(row=2, column=0, pady=5, sticky="e")
        category_var = tk.StringVar(value=values[1] if values else "")
        category_combo = ttk.Combobox(content_frame, textvariable=category_var, values=[cat[0] for cat in get_categories()] or ["Chưa có danh mục"], font=("Poppins", 12))
        category_combo.grid(row=2, column=1, pady=5, sticky="ew")

        tk.Label(content_frame, text="Số tiền:", bg="#F5F5F5", font=("Poppins", 12)).grid(row=3, column=0, pady=5, sticky="e")
        amount_entry = tk.Entry(content_frame, font=("Poppins", 12), bd=1, relief="solid")
        if values: amount_entry.insert(0, values[2])
        amount_entry.grid(row=3, column=1, pady=5, sticky="ew")

        tk.Label(content_frame, text="Ngày (YYYY-MM-DD):", bg="#F5F5F5", font=("Poppins", 12)).grid(row=4, column=0, pady=5, sticky="e")
        date_entry = tk.Entry(content_frame, font=("Poppins", 12), bd=1, relief="solid")
        if values: date_entry.insert(0, values[3])
        date_entry.grid(row=4, column=1, pady=5, sticky="ew")

        tk.Label(content_frame, text="Ghi chú:", bg="#F5F5F5", font=("Poppins", 12)).grid(row=5, column=0, pady=5, sticky="e")
        note_entry = tk.Entry(content_frame, font=("Poppins", 12), bd=1, relief="solid")
        if values: note_entry.insert(0, values[4])
        note_entry.grid(row=5, column=1, pady=5, sticky="ew")

        tk.Label(content_frame, text="Ví tiền:", bg="#F5F5F5", font=("Poppins", 12)).grid(row=6, column=0, pady=5, sticky="e")
        wallet_var = tk.StringVar(value=values[5] if values and values[5] in [w[1] for w in get_wallets()] else "")
        wallet_combo = ttk.Combobox(content_frame, textvariable=wallet_var, values=[f"{w[0]} - {w[1]}" for w in get_wallets()] or ["Chưa có ví"], font=("Poppins", 12))
        ...
        save_btn = tk.Button(form, text="Lưu", bg="#2ECC71", fg="white", font=("Poppins", 12, "bold"),
                             command=lambda: save_command(form, type_var, category_var, amount_entry, date_entry, note_entry, wallet_var),
                             width=15)
    ...
    def update_transaction(self, form, trans_id, type_var, category_var, amount_entry, date_entry, note_entry, wallet_var):
        try:
            type = type_var.get()
            category = category_var.get()
            amount = float(amount_entry.get())
